# R script to run bartMachine on Boston housing dataset with the same parameters as Python
...
# Set model parameters (same as Python)
num_trees <- 50
num_burn_in <- 100
...
write.csv(data.frame(
  index = test_idx,
  prediction = r_pred
), "r_bart_predictions.csv", row.names = FALSE)
...
), "r_bart_var_importance.csv", row.names = FALSE)

# Plot predictions vs actual
png("r_actual_vs_predicted.png", width = 800, height = 600)
plot(y_test, r_pred, main = "R BART: Actual vs Predicted", 
     xlab = "Actual Values", ylab = "Predicted Values", pch = 16, col = "blue")
abline(0, 1, col = "red", lty = 2)
dev.off()

# Plot variable importance
png("r_var_importance.png", width = 1000, height = 800)
barplot(importance, names.arg = var_names, main = "R BART: Variable Importance", 
        xlab = "Variables", ylab = "Importance (Proportion)", col = "blue")
dev.off()

cat("\nR implementation completed. Results saved to:\n")
cat("- r_bart_predictions.csv\n")
cat("- r_bart_var_importance.csv\n")
cat("- r_actual_vs_predicted.png\n")
cat("- r_var_importance.png\n")
