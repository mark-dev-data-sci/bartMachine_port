# R script to run bartMachine on Boston housing dataset with very long MCMC chains
...
train_idx <- read.csv("py_train_idx_very_long.csv")$train_idx
test_idx <- read.csv("py_test_idx_very_long.csv")$test_idx
...
# Build the model
cat("\nRunning R implementation with very long MCMC chains...\n")
start_time <- Sys.time()
r_bart <- bartMachine(
  X = X_train,
  y = y_train,
...
write.csv(data.frame(
  index = test_idx,
  prediction = r_pred
), "r_bart_predictions_very_long.csv", row.names = FALSE)
...
), "r_bart_var_importance_very_long.csv", row.names = FALSE)

# Plot predictions vs actual
png("r_actual_vs_predicted_very_long.png", width = 800, height = 600)
plot(y_test, r_pred, main = "R BART (Very Long MCMC): Actual vs Predicted", 
     xlab = "Actual Values", ylab = "Predicted Values", pch = 16, col = "blue")
abline(0, 1, col = "red", lty = 2)
dev.off()

# Plot variable importance
png("r_var_importance_very_long.png", width = 1000, height = 800)
barplot(importance, names.arg = var_names, main = "R BART (Very Long MCMC): Variable Importance", 
        xlab = "Variables", ylab = "Importance (Proportion)", col = "blue")
dev.off()

cat("\nR implementation with very long MCMC chains completed. Results saved to:\n")
cat("- r_bart_predictions_very_long.csv\n")
cat("- r_bart_var_importance_very_long.csv\n")
cat("- r_actual_vs_predicted_very_long.png\n")
cat("- r_var_importance_very_long.png\n")
